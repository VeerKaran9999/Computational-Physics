...
open(90,file="r2.dat", status="unknown")
open(82,file="energy_2.dat", status="unknown")
open(10,file="position_2.dat",status="unknown")
...
subroutine velocity_ani_force_intialization
    ...
    vel_const= dsqrt(12.0d0)*dsqrt(dfloat(kb_T))
    ...
    !write (*,*) avg_vx,avg_vy,avg_vz
     ...
     do i=1,n_part                                 !minus kelya mule he prove zal ki momentum conserve ahe
   ...
   ! avg_vx=0.0d0 ;avg_vy=0.0d0 ;avg_vz=0.0d0
    ...
    do j=i+1, n_part                                       !differance find kela ahe postion madhala
        ...
        if(abs(x).gt.(dfloat(lx)/2.0d0))  x=(dfloat(lx)-abs(x))*((-1.0d0*x)/abs(x))
        if(abs(y).gt.(dfloat(ly)/2.0d0))  y=(dfloat(ly)-abs(y))*((-1.0d0*y)/abs(y))
        if(abs(z).gt.(dfloat(lz)/2.0d0))   z=(dfloat(lz)-abs(z))*((-1.0d0*z)/abs(z))
   ...
   ! acc= force/dfloat(mass)
    ...
    do j=i+1, n_part                                       !differce find kela ahe postion madhala
        ...
        if (r<=rc ) then
            ...
            new_force(3*i-2) =force(3*i-2) + (lj_force)*(x/r)
            new_force(3*i-1) =force(3*i-1) + (lj_force)*(y/r)   !eka particle var bakichya particle mule lagnara force ani (x/r ne multiply kel ahe becouse direction sathi 
            new_force(3*i)   =force(3*i)   + (lj_force)*(z/r)
            new_force(3*j-2) =force(3*j-2) - (lj_force)*(x/r)   
            new_force(3*j-1) =force(3*j-1) - (lj_force)*(y/r)   ! equal and opposite force lagele dusarya particle var
            new_force(3*j)   =force(3*j)   - (lj_force)*(z/r)
  ...
  do i= 1, n_part
   ...
   ! avr_vel_x= avr_vel_x + vel(3*i-2)
    ...
    ke= ke + vel(3*i)*vel(3*i)
              ...
              ! next itteration sathi old force ha equat karto apan new force shi

    
 end do
    
 do j=1,n_part
    force(3*j-2) = new_force(3*j-2) 
    force(3*j-1) = new_force(3*j-1)  
    force(3*j)   = new_force(3*j)   
 end do
